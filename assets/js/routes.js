$("#coruscant, #coruscantlabel").click(function(){
    location.href = "./Planets/coruscant.html";
});
$("#hosnianprime, #hosnianprimelabel").click(function(){
    location.href = "./Planets/hosnianprime.html";
});
$("#corellia, #corellialabel").click(function(){
    location.href = "./Planets/corellia.html";
});
$("#alderanintact, #alderandestroyed, #alderanlabel").click(function(){
    location.href = "./Planets/alderan.html";
});
$("#takodana, #takodanalabel").click(function(){
    location.href = "./Planets/takodana.html";
});
$("#jakku, #jakkulabel").click(function(){
    location.href = "./Planets/jakku.html";
});
$("#onderon, #onderonlabel").click(function(){
    location.href = "./Planets/onderon.html";
});
$("#mimban, #mimbanlabel").click(function(){
    location.href = "./Planets/mimban.html";
});
$("#vandor, #vandorlabel").click(function(){
    location.href = "./Planets/vandor.html";
});
$("#shill, #shilllabel").click(function(){
    location.href = "./Planets/shill.html";
});
$("#kashyyk, #kashyyklabel").click(function(){
    location.href = "./Planets/kashyyk.html";
});
$("#dathomir, #dathomirlabel").click(function(){
    location.href = "./Planets/dathomir.html";
});
$("#krownest, #krownestlabel").click(function(){
    location.href = "./Planets/krownest.html";
});
$("#concorddawn, #concorddawnlabel").click(function(){
    location.href = "./Planets/concorddawn.html";
});
$("#mandalore, #mandalorelabel").click(function(){
    location.href = "./Planets/mandalore.html";
});
$("#wobani, #wobanilabel").click(function(){
    location.href = "./Planets/wobani.html";
});
$("#naboo, #naboolabel").click(function(){
    location.href = "./Planets/naboo.html";
});
$("#jedha, #jedhalabel").click(function(){
    location.href = "./Planets/jedha.html";
});
$("#agamar, #agamarlabel").click(function(){
    location.href = "./Planets/agamar.html";
});
$("#scariff, #scarifflabel").click(function(){
    location.href = "./Planets/scariff.html";
});
$("#tatooine, #tatooinelabel").click(function(){
    location.href = "./Planets/tatooine.html";
});
$("#genosis, #genosislabel").click(function(){
    location.href = "./Planets/genosis.html";
});
$("#dqar, #dqarlabel").click(function(){
    location.href = "./Planets/dqar.html";
});
$("#crait, #craitlabel").click(function(){
    location.href = "./Planets/crait.html";
});
$("#hoth, #hothlabel").click(function(){
    location.href = "./Planets/hoth.html";
});
$("#bespin, #bespinlabel").click(function(){
    location.href = "./Planets/bespin.html";
});
$("#endor, #endorlabel").click(function(){
    location.href = "./Planets/moonofendor.html";
});
$("#datooine, #datooinelabel").click(function(){
    location.href = "./Planets/datooine.html";
});
$("#yavin, #yavinlabel").click(function(){
    location.href = "./Planets/yavin4.html";
});
$("#stygeonprime, #stygeonprimelabel").click(function(){
    location.href = "./Planets/stygeonprime.html";
});
$("#kessel, #kessellabel").click(function(){
    location.href = "./Planets/kessel.html";
});
$("#dagobah, #dagobahlabel").click(function(){
    location.href = "./Planets/dagobah.html";
});
$("#lahmu, #lahmulabel").click(function(){
    location.href = "./Planets/lahmu.html";
});
$("#seelos, #seeloslabel").click(function(){
    location.href = "./Planets/seelos.html";
});
$("#atollon, #atollonlabel").click(function(){
    location.href = "./Planets/atollon.html";
});
$("#eadu, #eadulabel").click(function(){
    location.href = "./Planets/eadu.html";
});
$("#kamino, #kaminolabel").click(function(){
    location.href = "./Planets/kamino.html";
});
$("#ryloth, #rylothlabel").click(function(){
    location.href = "./Planets/ryloth.html";
});
$("#utapau, #utapaulabel").click(function(){
    location.href = "./Planets/utapau.html";
});
$("#mustafar, #mustafarlabel").click(function(){
    location.href = "./Planets/mustafar.html";
});
$("#starkillerbase, #starkillerbaselabel").click(function(){
    location.href = "./Planets/starkillerbase.html";
});